...
## WP16 npm graph (`src/web-editor`)

Direct, exact versions (no caret ranges). Transitives are owned by the `pnpm-lock.yaml` generated by pnpm 11.19.0; do not hand-edit the lockfile.

| Package | Approved version | Purpose | License | Security/update owner | Replacement boundary |
|---|---:|---|---|---|---|
| `@codemirror/state` | 6.7.1 | CodeMirror 6 editor state | MIT | LLMW.Writing maintainers | `src/web-editor` renderer bundle only |
| `@codemirror/view` | 6.43.7 | CodeMirror 6 editor view | MIT | LLMW.Writing maintainers | `src/web-editor` renderer bundle only |
| `@codemirror/commands` | 6.10.4 | History, keymap, undo/redo | MIT | LLMW.Writing maintainers | `src/web-editor` renderer bundle only |
| `@codemirror/lang-markdown` | 6.5.1 | Markdown highlighting (not HTML authority) | MIT | LLMW.Writing maintainers | `src/web-editor` renderer bundle only |
| `esbuild` | 0.28.1 | Local browser bundler (devDependency) | MIT | LLMW.Writing maintainers | `src/web-editor` build scripts only |

Not added: `codemirror` meta-package, React/Vue/Svelte/Angular, Vite/Webpack/Rollup, TypeScript/Babel, jsdom/Playwright/Jest/Vitest, Monaco, Ace, Electron.

Toolchain: Node.js 24 LTS; pnpm 11.19.0 via Corepack (`packageManager` pin). No `pnpm@latest`.

Lock file: repository-root `pnpm-lock.yaml` (pnpm workspace including `src/web-editor`).

WP16 audit commands (run during the work package, do not ignore high/critical):

- `pnpm --dir src/web-editor list --depth Infinity`
- `pnpm --dir src/web-editor audit --audit-level high`

Audit result (2026-08-18, Node v24.19.0, pnpm 11.19.0): `pnpm --dir src/web-editor audit --audit-level high` reported **No known vulnerabilities found**. Workspace graph is 94 packages (CodeMirror/Lezer plus esbuild platform optional packages). No high/critical issues. No new NuGet packages. WindowsAppSDK remains 2.4.0.
